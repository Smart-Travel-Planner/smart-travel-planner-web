...
describe('ActivityListComponent', () => {
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: { getTripById: vi.fn().mockReturnValue(of(mockTrip)) },
        },
        { provide: GeocodingService, useValue: geocodingServiceMock },
        {
  ...
  it('should set tripId on init', () => {
    expect(component.tripId()).toBe('trip1');
  });

  it('should resolve destination coords on init', () => {
    expect(geocodingServiceMock.getDestinationOrUserCoords).toHaveBeenCalledWith('París');
    expect(component.tripDestinationCoords()).toEqual({ lat: 48.8566, lng: 2.3522 });
  });
});
